...
def encode_inform(config, data):
    ...
    test1 = config.get('gateway', 'key')
    test = a2b_hex(config.get('gateway', 'key'))
...
def _create_complete_inform(config):
    return json.dumps({
        ...
        "board_rev": 7,
        "bootid": 0,
        "bootrom_version": "usw-v1.1.4.115-g14af1ee6",
        'cfgversion':  '' if not config.has_section('mgmt_cfg') else not config.get('mgmt_cfg', 'cfgversion'),
        # 'config_network_wan': {
        #     'type': 'dhcp',
        # },
        # 'config_port_table': [
        ...
        'isolated': False,
        'locating': True,
        'mac': config.get('gateway', 'lan_mac').lower(),
        'model': 'UGW3',
        ...
        #"serial": "788A20DFED17",
        "service_mac":config.get('gateway', 'lan_mac').lower(),
        ...
        #         'addresses': [
        #             '%s/24' % config.get('gateway', 'lan_ip')
        #         ],
        #         'autoneg': 'true',
        #         'duplex': 'full',
        #         'host_table': [
